...
Emitter.prototype.getShape = function() {
	var d = new Date(),
		lessSeconds = Math.round((endData.getTime() - d.getTime())/1000),
		realLessSeconds = lessSeconds>0?lessSeconds:0;
		hourTemp = parseInt(realLessSeconds/3600),
		minTemp = parseInt((realLessSeconds - hourTemp * 3600)/60),
		secTemp = parseInt(realLessSeconds%60),
		hour = hourTemp>=10?hourTemp:("0"+hourTemp),
		min = minTemp>=10?minTemp:("0"+minTemp),
		sec = secTemp>=10?secTemp:("0"+secTemp),
